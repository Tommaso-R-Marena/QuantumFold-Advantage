...
class InvariantPointAttention(nn.Module):
    ...
    def __init__(
        self, dim: int = 256, n_heads: int = 8, n_query_points: int = 4, n_value_points: int = 8
    ):
        super().__init__()
        self.dim = dim
    ...
    def forward(
        self, features: torch.Tensor, frames: torch.Tensor, mask: Optional[torch.Tensor] = None
...
class EquivariantGraphConv(nn.Module):
    ...
    def __init__(self, in_dim: int = 128, hidden_dim: int = 128, out_dim: int = 128):
        ...
        self.node_mlp = nn.Sequential(
            nn.Linear(in_dim + hidden_dim, hidden_dim), nn.SiLU(), nn.Linear(hidden_dim, out_dim)
        )
...
class StructureModule(nn.Module):
    ...
    def __init__(
        self, dim: int = 256, n_layers: int = 8, use_ipa: bool = True, use_egnn: bool = True
    ):
        super().__init__()
        self.dim = dim
        ...
        self.coord_head = nn.Sequential(nn.Linear(dim, dim), nn.GELU(), nn.Linear(dim, 3))
...
class QuantumEnhancedProteinFolder(nn.Module):
    ...
    def __init__(
        ...
        self.structure_module = StructureModule(
            dim=hidden_dim, n_layers=n_structure_layers, use_ipa=True, use_egnn=True
        )
