...
class ProteinVisualizer:
    ...
    def plot_contact_map(
        ...
        # Plot contact map
        im = ax.imshow(
            contacts, cmap='RdBu_r', aspect='equal',
    ...
    def plot_attention_heatmap(
        ...
        # Plot heatmap
        sns.heatmap(
            attn, cmap='YlOrRd', square=True,
            cbar_kws={'label': 'Attention Weight'},
            ax=ax, vmin=0, vmax=attn.max()
        )
            ...
            for qubit in range(num_qubits):
                if gate_idx < len(gate_sequence):
                    ...
                    # Two qubit gate
                        ...
                        ax.text(
                            depth, qubit + 1, '⊕' if gate == 'CNOT' else 'Z',
    ...
    def _plot_ramachandran_background(self, ax):
        ...
        alpha_region = mpatches.Rectangle(
            (-90, -60), 40, 80,
            facecolor='#E8F4F8', edgecolor='#B0D4E3', alpha=0.5, linewidth=2
        )
        ...
        beta_region = mpatches.Rectangle(
            (-180, 100), 100, 60,
            facecolor='#FFF4E6', edgecolor='#FFD699', alpha=0.5, linewidth=2
        )
            ...
            for i, ss in enumerate(ss_string):
                ...
                if axis == 'x':
                    ax.add_patch(mpatches.Rectangle(
                        (i, -length * 0.02), 1, length * 0.015,
                        facecolor=color, edgecolor='none'
                    ))
                else:
                    ax.add_patch(mpatches.Rectangle(
                        (-length * 0.02, i), length * 0.015, 1,
                        facecolor=color, edgecolor='none'
                    ))
