...
class PublicationFigures:
    ...
    def plot_benchmark_comparison(self, results: Dict[str, Dict], save_name: str = "benchmark_comparison"):
        ...
        plt.suptitle('Performance Comparison Across Methods', fontsize=16, fontweight='bold', y=0.98)
    ...
    def plot_training_curves(self, history: Dict[str, List[float]], save_name: str = "training_curves"):
        ...
        if 'val_tm_score' in history:
            axes[2].plot(history['val_tm_score'], label='Validation TM-score', linewidth=2, color='green')
    ...
    def plot_structure_comparison(self, predicted: np.ndarray, target: np.ndarray,
                                 pdb_id: str, save_name: Optional[str] = None):
        ...
        ax1.plot(predicted[:, 0], predicted[:, 1], predicted[:, 2],
                'b-', alpha=0.6, linewidth=2, label='Backbone')
        ax1.scatter(predicted[:, 0], predicted[:, 1], predicted[:, 2],
                   c=np.arange(len(predicted)), cmap='viridis', s=30, alpha=0.7)
        ...
        ax2.plot(target[:, 0], target[:, 1], target[:, 2],
                'r-', alpha=0.6, linewidth=2, label='Backbone')
        ax2.scatter(target[:, 0], target[:, 1], target[:, 2],
                   c=np.arange(len(target)), cmap='plasma', s=30, alpha=0.7)
        ...
        ax3.plot(predicted[:, 0], predicted[:, 1], predicted[:, 2],
                'b-', alpha=0.5, linewidth=2, label='Predicted')
        ax3.plot(target[:, 0], target[:, 1], target[:, 2],
                'r-', alpha=0.5, linewidth=2, label='Target')
        ax3.scatter(predicted[:, 0], predicted[:, 1], predicted[:, 2],
    ...
    def plot_per_residue_error(self, predicted: np.ndarray, target: np.ndarray,
                              sequence: str, pdb_id: str, save_name: Optional[str] = None):
        ...
        axes[0].axhline(y=np.mean(distances), color='red', linestyle='--',
    ...
    def plot_distance_matrix_comparison(self, predicted: np.ndarray, target: np.ndarray,
                                       pdb_id: str, save_name: Optional[str] = None):
    ...
    def plot_metric_distributions(self, results: Dict[str, List[float]], save_name: str = "metric_distributions"):
