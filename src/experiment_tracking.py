...
class ExperimentTracker:
    ...
    def __init__(self, config: ExperimentConfig, project_name: str = "QuantumFold-Advantage",
                 entity: Optional[str] = None, tags: Optional[List[str]] = None,
                 notes: Optional[str] = None, enable_wandb: bool = True):
        """
    ...
    def log_best_model(self, model: torch.nn.Module, metric_value: float,
                      metric_name: str, higher_is_better: bool = False):
    ...
    def log_predictions(self, predictions: np.ndarray, targets: np.ndarray,
                       pdb_ids: List[str], step: int, max_samples: int = 10):
        ...
        for i in range(n_samples):
            ...
            ax3.plot(pred[:, 0], pred[:, 1], pred[:, 2], 'b-', alpha=0.4, linewidth=2, label='Predicted')
            ax3.plot(targ[:, 0], targ[:, 1], targ[:, 2], 'r-', alpha=0.4, linewidth=2, label='Target')
    ...
    def log_attention_maps(self, attention_weights: torch.Tensor, sequence: str,
                          step: int, layer_idx: int = 0, head_idx: int = 0):
