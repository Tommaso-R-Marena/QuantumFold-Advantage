...
class CASPDataset:
    ...
    def __init__(self, casp_version: int = 14, data_dir: str = "data/casp", download: bool = True):
    ...
    def get_target(self, target_id: str) -> Tuple[str, np.ndarray]:
        ...
        if target_id not in self.structures:
            raise ValueError(
                f"Target {target_id} not found. Available: {list(self.structures.keys())[:10]}..."
            )
...
class CASPBenchmark:
    ...
    def benchmark_casp(self, casp_version: int = 14, max_targets: Optional[int] = None) -> Dict:
        ...
        results = {"tm_scores": [], "rmsd_scores": [], "gdt_ts_scores": [], "per_target": {}}
        ...
        if results["tm_scores"]:
            ...
            print(
                f"RMSD:     {results['summary']['rmsd_mean']:.2f} ± {results['summary']['rmsd_std']:.2f} Å"
            )
            print(
                f"GDT-TS:   {results['summary']['gdt_ts_mean']:.4f} ± {results['summary']['gdt_ts_std']:.4f}"
            )
            print(f"{'='*80}\n")
