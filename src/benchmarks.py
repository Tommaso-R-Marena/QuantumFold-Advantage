...
class ProteinStructureEvaluator:
    ...
    def calculate_rmsd(
        self, coords_pred: np.ndarray, coords_true: np.ndarray, align: bool = True
    ) -> float:
    ...
    def calculate_lddt(
        ...
        dist_true = np.sqrt(
            np.sum((coords_true[:, None, :] - coords_true[None, :, :]) ** 2, axis=2)
        )
        dist_pred = np.sqrt(
            np.sum((coords_pred[:, None, :] - coords_pred[None, :, :]) ** 2, axis=2)
        )
    ...
    def calculate_clash_score(self, coords: np.ndarray, clash_threshold: float = 2.0) -> float:
    ...
    def _align_structures(self, coords_mobile: np.ndarray, coords_target: np.ndarray) -> np.ndarray:
