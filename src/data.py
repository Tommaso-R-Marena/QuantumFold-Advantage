...
class ProteinDataset(Dataset):
    ...
    def __init__(self, sequences: List[str], coordinates: List[np.ndarray], labels: List[int] = None):
        """
        Initialize dataset.
            ...
            for model in structure:
                for chain in model:
                    ...
                    if len(residues) > max_length:
                        logger.warning(f"{pdb_id} chain {chain.id}: {len(residues)} residues > {max_length}, skipping")
                        continue
                    
                    # Extract sequence
                    seq = "".join([PDB.Polypeptide.three_to_one(res.get_resname()) for res in residues])
...
def load_data(use_real_data: bool = True, num_synthetic: int = 20, seed: int = 42) -> Tuple[ProteinDataset, ProteinDataset]:
    """
