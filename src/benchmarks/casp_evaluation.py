...
class CASPEvaluator:
    ...
    def kabsch_superimpose(
        self, mobile: np.ndarray, target: np.ndarray
    ...
    def gdt_score(
        self, pred_coords: np.ndarray, true_coords: np.ndarray, cutoffs: List[float]
    ) -> float:
    ...
    def gdt_ts(self, pred_coords: np.ndarray, true_coords: np.ndarray) -> float:
        """GDT_TS (Total Score) - average of 1, 2, 4, 8 Å cutoffs."""
        return self.gdt_score(pred_coords, true_coords, [1.0, 2.0, 4.0, 8.0])

    def gdt_ha(self, pred_coords: np.ndarray, true_coords: np.ndarray) -> float:
    ...
    def lddt(
        ...
        for i in range(n_residues):
            ...
            neighbors = np.where((true_dists[i] > 0) & (true_dists[i] < inclusion_radius))[0]
    ...
    def lga_score(
        self, pred_coords: np.ndarray, true_coords: np.ndarray, cutoff: float = 5.0
    ) -> float:
    ...
    def aggregate_results(
        self, metrics_list: List[StructureMetrics]
        ...
        for key in metrics_list[0].to_dict().keys():
            values = [
                getattr(m, key.lower().replace("-", "_").replace("_", "_")) for m in metrics_list
            ]
