...
class ESM2Embedder(nn.Module):
    ...
    def forward(
        ...
        output: Dict[str, Any] = {"embeddings": embeddings, "mean_embedding": mean_embedding}
...
class EvolutionaryFeatureExtractor(nn.Module):
    ...
    def compute_pssm(self, msa: np.ndarray, pseudocount: float = 0.001) -> np.ndarray:
    ...
    def compute_conservation(self, msa: np.ndarray) -> np.ndarray:
    ...
    def compute_coevolution(self, msa: np.ndarray, apc_correction: bool = True) -> np.ndarray:
...
class GeometricFeatureExtractor(nn.Module):
    ...
    def compute_torsion_angles(self, coords: Tensor) -> Tensor:
    ...
    def compute_local_frames(self, coords: Tensor) -> Tuple[Tensor, Tensor]:
    ...
    def forward(self, coords: Tensor) -> Dict[str, Tensor]:
        ...
        return {"angles": angles, "origins": origins, "frames": frames, "distances": distances}
...
class CombinedProteinEmbedding(nn.Module):
    ...
    def forward(self, sequences: List[str], msa: Optional[np.ndarray] = None) -> Tensor:
