...
class InvariantPointAttention(nn.Module):
    ...
    def forward(
        ...
        q_points = self.linear_q_points(s).view(batch_size, n_res, self.n_heads, self.n_query_points, 3)
        k_points = self.linear_k_points(s).view(batch_size, n_res, self.n_heads, self.n_query_points, 3)
        
        # Transform points to global frame
        q_points_global = torch.einsum('bnij,bnhpj->bnhpi', rotations, q_points) + translations.unsqueeze(2).unsqueeze(2)
        k_points_global = torch.einsum('bnij,bnhpj->bnhpi', rotations, k_points) + translations.unsqueeze(2).unsqueeze(2)
        ...
        point_distances = torch.sum(
            (q_points_global.unsqueeze(2) - k_points_global.unsqueeze(1)) ** 2,
            dim=-1
        ...
        # Point values
        v_points = self.linear_v_points(s).view(batch_size, n_res, self.n_heads, self.n_point_values, 3)
        v_points_global = torch.einsum('bnij,bnhpj->bnhpi', rotations, v_points) + translations.unsqueeze(2).unsqueeze(2)
        
        v_out_points = torch.einsum('bnmh,bmhpi->bnhpi', attn_weights, v_points_global)
        
        # Transform back to local frame
        v_out_points_local = torch.einsum('bnij,bnhpj->bnhpi', 
...
class StructureModule(nn.Module):
    ...
    def __init__(self, c_s: int = 384, n_layers: int = 8):
        ...
        self.ipa_layers = nn.ModuleList([
            InvariantPointAttention(c_s=c_s)
    ...
    def _compute_rigids(self, coords: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        rotations = torch.eye(3, device=coords.device).unsqueeze(0).unsqueeze(0).repeat(batch_size, n_res, 1, 1)
...
class AdvancedProteinFoldingModel(nn.Module):
    ...
    def __init__(
        ...
        self.pair_embed = nn.Sequential(
            nn.Linear(c_s * 2 + 1, c_z),  # +1 for relative position
            nn.ReLU(),
            nn.Linear(c_z, c_z)
        )
