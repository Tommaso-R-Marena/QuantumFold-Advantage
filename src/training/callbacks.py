...
class EarlyStopping(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if current is None:
            warnings.warn(
                f"Early stopping requires {self.monitor} available. Skipping.", RuntimeWarning
            )
            return
        ...
        if self.monitor_op(current, self.best_value):
            ...
            if self.restore_best_weights:
                self.best_weights = {
                    k: v.cpu().clone() for k, v in logs.get("model_state", {}).items()
                }
        else:
            self.wait += 1
...
class ProgressCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        print(
            f"\rEpoch {epoch + 1}/{self.total_epochs} [{progress:>5.1f}%] - {metrics_str}", end=""
        )
