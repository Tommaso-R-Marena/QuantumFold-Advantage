...
class FAPELoss(nn.Module):
    ...
    def forward(
        ...
        # Vectorized transform: x_local = R^T @ (x_global - t)
        ...
        # Apply rotations: (B, F, 3, 3) @ (B, F, L, A, 3) -> (B, F, L, A, 3)
        # We swap i and j in einsum to use transpose of R
        ...
        if mask is not None:
            ...
            sum_bla = dist.sum(dim=(0, 2, 3))
            loss = (sum_bla / (mask.sum() * A + self.eps)).mean()
        else:
            loss = dist.mean()
