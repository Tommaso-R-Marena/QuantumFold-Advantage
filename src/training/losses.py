...
class FAPELoss(nn.Module):
    ...
    def forward(
        ...
        # Vectorized implementation for speed: (B, n_frames, L, A, 3)
        ...
        # Transform atoms: R^T @ (x - t)
        # Broadcasting: (B, 1, L, A, 3) - (B, n_frames, 1, 1, 3) -> (B, n_frames, L, A, 3)
        pred_local = torch.einsum(
            "bfij,bflaj->bflai",
            R_pred.transpose(-1, -2),
            pred_coords.unsqueeze(1) - t_pred.unsqueeze(2).unsqueeze(3),
        )
        true_local = torch.einsum(
            "bfij,bflaj->bflai",
            R_true.transpose(-1, -2),
            true_coords.unsqueeze(1) - t_true.unsqueeze(2).unsqueeze(3),
        )
        ...
        if mask is not None:
            # mask: (B, L) -> (B, 1, L, 1) to broadcast with (B, n_frames, L, A)
            dist = dist * mask.unsqueeze(1).unsqueeze(-1).float()
            # Mean over atoms, sequences, and sampled frames
            return dist.sum() / (mask.sum() * A * n_frames + self.eps)
