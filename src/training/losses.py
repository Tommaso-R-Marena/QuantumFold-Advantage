...
class FAPELoss(nn.Module):
    ...
    def forward(
        ...
        # For computational efficiency, sample a subset of frames (up to 32)
        L = pred_coords.shape[1]
        n_frames = min(L, 32)
        frame_idx = torch.linspace(0, L - 1, n_frames, device=pred_coords.device).long()

        # Vectorized implementation (B, F, L, A, 3)
        # 1. Select frames: (B, F, 3, 3) and (B, F, 3)
        ...
        # 2. Transform into local frames: R^T @ (x - t)
        # Using einsum for (B, F, 3, 3) @ (B, L, A, 3) -> (B, F, L, A, 3)
        # x_shifted: (B, F, L, A, 3)
        x_shifted_pred = pred_coords.unsqueeze(1) - t_pred.unsqueeze(2).unsqueeze(3)
        x_shifted_true = true_coords.unsqueeze(1) - t_true.unsqueeze(2).unsqueeze(3)

        pred_local = torch.einsum("bfij,bflaj->bflai", R_pred.transpose(-1, -2), x_shifted_pred)
        true_local = torch.einsum("bfij,bflaj->bflai", R_true.transpose(-1, -2), x_shifted_true)

        # 3. Compute distances and clamp: (B, F, L, A)
        dist = torch.sqrt(torch.sum((pred_local - true_local) ** 2, dim=-1) + self.eps)
        dist = torch.clamp(dist, max=self.d_clamp)

        # 4. Average over atoms, sequence, and frames
        if mask is not None:
            # mask: (B, L) -> (B, 1, L, 1) for broadcasting
            m = mask.unsqueeze(1).unsqueeze(-1).float()
            # Average over non-masked positions
            loss = (dist * m).sum() / (dist.shape[1] * m.sum() * dist.shape[3] + self.eps)
        else:
            loss = dist.mean()
