...
class StructureValidator:
    ...
    def validate_ramachandran(
        ...
        warnings.warn(
            "Full Ramachandran validation not implemented. "
            "Use Bio.PDB for complete validation."
        )
    ...
    def check_structure_quality(
        ...
        coord_results = StructureValidator.validate_coordinates(
...
def sanitize_predictions(
    ...
    if 'coordinates' in predictions:
        ...
        if coords.shape[-2] != seq_len:
            warnings.warn(
                f"Coordinate shape mismatch. Expected {seq_len}, got {coords.shape[-2]}"
            )
