...
def validate_tensor(
    ...
    if dtype is not None and tensor.dtype != dtype:
        raise ValidationError(f"{name} must have dtype {dtype}, got {tensor.dtype}")
    ...
    if ndim is not None and tensor.ndim != ndim:
        raise ValidationError(f"{name} must have {ndim} dimensions, got {tensor.ndim}")
    ...
    if min_value is not None and tensor.min() < min_value:
        raise ValidationError(f"{name} minimum value {tensor.min():.6f} is less than {min_value}")

    if max_value is not None and tensor.max() > max_value:
        raise ValidationError(f"{name} maximum value {tensor.max():.6f} exceeds {max_value}")
...
def validate_type(value: Any, name: str, expected_type: Union[type, Tuple[type, ...]]) -> Any:
    ...
    if not isinstance(value, expected_type):
        raise ValidationError(f"{name} must be of type {expected_type}, got {type(value)}")
    return value
...
def validate_config(config: dict, required_keys: List[str]) -> dict:
    ...
    if missing_keys:
        raise ValidationError(f"Configuration missing required keys: {missing_keys}")
    return config
...
def validate_protein_sequence(sequence: str) -> str:
    ...
    if invalid_chars:
        raise ValidationError(f"Invalid amino acids in sequence: {invalid_chars}")

    return sequence


def validate_coordinates(
    coords: torch.Tensor, num_atoms: Optional[int] = None, allow_nan: bool = False
) -> torch.Tensor:
    ...
    if coords.shape[-1] != 3:
        raise ValidationError(f"Coordinates must have 3 dimensions (x,y,z), got {coords.shape[-1]}")

    # Check number of atoms if specified
    if num_atoms is not None:
        actual_atoms = coords.shape[-2]
        if actual_atoms != num_atoms:
            raise ValidationError(f"Expected {num_atoms} atoms, got {actual_atoms}")

    return coords


def safe_divide(
    numerator: torch.Tensor, denominator: torch.Tensor, epsilon: float = 1e-10
) -> torch.Tensor:
...
def clip_gradients(
    parameters, max_norm: float = 1.0, norm_type: float = 2.0, error_if_nonfinite: bool = True
) -> torch.Tensor:
    ...
    total_norm = torch.nn.utils.clip_grad_norm_(
        parameters, max_norm, norm_type=norm_type, error_if_nonfinite=False
    )
