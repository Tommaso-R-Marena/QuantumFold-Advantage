...
class HyperparameterTuner:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized tuner: {study_name}, " f"direction={direction}, sampler={sampler_name}"
        )
    ...
    def objective(
        self, trial: optuna.Trial, train_fn: Callable, config: Optional[Dict] = None
    ) -> float:
...
def create_training_objective(
    model_class, dataset, n_epochs: int = 10, device: str = "cuda"
) -> Callable:
