...
class SequenceProcessor:
    ...
    def decode_sequence(encoded: torch.Tensor) -> str:
        ...
        return ''.join(SequenceProcessor.IDX_TO_AA.get(int(idx), 'X') 
                      for idx in encoded)
...
class BatchCollator:
    ...
    def __call__(self, batch: List[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'coordinates' in batch[0]:
            ...
            for i, item in enumerate(batch):
                length = len(item['sequence'])
                padded_coords[i, :length] = torch.from_numpy(
                    item['coordinates']
                ).float()
...
def length_based_bucketing(
    ...
    sorted_items = sorted(
        items,
        key=lambda x: len(x[length_key]) if isinstance(x[length_key], (str, list)) 
                     else x[length_key].shape[0]
    )
