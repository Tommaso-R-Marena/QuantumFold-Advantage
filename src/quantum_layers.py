...
class AdvancedQuantumCircuitLayer(nn.Module):
    ...
    def __init__(
        ...
        self.total_params = self.n_params_per_layer * n_layers + n_qubits  # +n_qubits for input encoding
        ...
        self.param_scaling = nn.Parameter(
            torch.ones(n_layers) * (1.0 / np.sqrt(n_layers)),
            requires_grad=True
        )
