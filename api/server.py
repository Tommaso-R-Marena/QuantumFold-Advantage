    ...
    try:
        ...
        if Path(checkpoint_path).exists():
            state.pipeline = QuantumFoldPipeline(
                checkpoint=checkpoint_path, use_quantum=True, device=state.device
            )
...
async def run_prediction(job_id: str):
    ...
    try:
        ...
        if job["save_pdb"]:
            ...
            state.pipeline.save_structure(coordinates, str(pdb_path), sequence=sequence)
...
async def download_file(filename: str):
    ...
    return FileResponse(path=file_path, media_type="chemical/x-pdb", filename=filename)
...
async def evaluate_prediction(
    predicted_file: UploadFile = File(...), ground_truth_file: UploadFile = File(...)
):
    ...
    try:
        ...
        metrics = state.evaluator.evaluate_structure(
            coords_pred, coords_true, sequence_length=len(coords_true)
        )
