...
class Test3DVisualization:
    ...
    def test_backbone_trace_plot(self, sample_coords):
        ...
        ax.scatter(sample_coords[:, 0], sample_coords[:, 1], sample_coords[:, 2], c=range(len(sample_coords)))
...
class TestDistanceMaps:
    ...
    def test_distance_matrix_calculation(self, sample_coords):
        ...
        distances = np.sqrt(np.sum((sample_coords[:, None, :] - sample_coords[None, :, :]) ** 2, axis=2))
    ...
    def test_distance_difference_map(self, sample_coords):
        ...
        distances1 = np.sqrt(np.sum((sample_coords[:, None, :] - sample_coords[None, :, :]) ** 2, axis=2))
