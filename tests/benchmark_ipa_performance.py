...
def benchmark_sequence_lengths():
    ...
    for seq_len in [128, 256, 512, 768]:
        ...
        if seq_len > 500:
            assert model.structure_module.ipa_layers[0].use_checkpointing, \
                "Checkpointing should be enabled for seq_len > 500"
    ...
    print("-" * 60)
    for r in results:
        print(f"{r['seq_len']:<10} {r['time_sec']:<12.3f} {r['memory_gb']:<15.2f} {r['time_per_residue_ms']:<12.2f}")
