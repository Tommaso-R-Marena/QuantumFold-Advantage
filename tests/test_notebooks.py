        ...
        for i, cell in enumerate(nb.cells):
            ...
            for pattern in sensitive_patterns:
                if re.search(pattern, source, re.I):
                    ...
                    if 'your_' not in source.lower() and \
                       'example' not in source.lower() and \
    ...
    def test_colab_compatibility(self, notebook_path):
        ...
        for cell in nb.cells:
            if cell.cell_type == 'markdown':
                source = ''.join(cell.source)
                if 'colab.research.google.com' in source or \
                   'Open In Colab' in source:
                    has_colab_badge = True
                    break
        
        # Production notebooks should have Colab badges
        if 'production' in str(notebook_path).lower() or \
           'a100' in str(notebook_path).lower():
            assert has_colab_badge, \
...
class TestImports:
    ...
    def test_no_missing_src_imports(self, notebook_path):
        ...
        for module in src_imports:
            ...
            assert module_path.exists() or package_path.exists(), \
