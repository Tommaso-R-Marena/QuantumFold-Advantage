...
def execute_notebook(notebook_path, timeout=TIMEOUT, kernel_name="python3"):
    ...
    ep = ExecutePreprocessor(
        timeout=timeout, kernel_name=kernel_name, allow_errors=False  # Stop on first error
    )
    ...
    try:
        ...
        raise NotebookExecutionError(f"Failed to execute {notebook_path.name}: {str(e)}") from e
...
class TestNotebookExecution:
    ...
    def test_colab_quickstart_executes(self):
        ...
        assert (
            stats["cells_with_errors"] == 0
        ), f"Found {stats['cells_with_errors']} cells with errors"
    ...
    @pytest.mark.skipif(
        not (
            Path(__file__).parent.parent / "examples" / "03_advanced_visualization.ipynb"
        ).exists(),
        reason="Notebook not found",
    )
