...
def parse_args():
    ...
    parser.add_argument(
        "--data_dir", type=str, required=True, help="Directory containing validation data"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--use_quantum", action="store_true", help="Use quantum layers (must match training)"
    )
...
def main():
    ...
    model = QuantumFoldModel(
        n_layers=6, embed_dim=128, n_heads=8, use_quantum=args.use_quantum, n_qubits=4
    ).to(args.device)
