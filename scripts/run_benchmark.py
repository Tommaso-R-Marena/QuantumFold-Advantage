...
def parse_args():
    ...
    parser.add_argument(
        "--compare_alphafold", action="store_true", help="Include AlphaFold-3 comparison"
    )
    parser.add_argument(
        "--alphafold_dir", type=str, help="Directory containing AlphaFold-3 predictions"
    )
    parser.add_argument(
        "--device",
        type=str,
...
def main():
    ...
    model = QuantumFoldModel(n_layers=6, embed_dim=128, n_heads=8, use_quantum=True, n_qubits=4).to(
        args.device
    )
    ...
    for i, protein_data in enumerate(test_proteins):
        ...
        try:
            ...
            coords_quantumfold = run_quantumfold_inference(model, protein_data, args.device)
            ...
            for method, metrics in comparison.items():
                print(
                    f"  {method}: TM-score={metrics.tm_score:.3f}, "
