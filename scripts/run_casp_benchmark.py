...
def main():
    ...
    parser.add_argument(
        "--esm-model", type=str, default="esm2_t33_650M_UR50D", help="ESM-2 model variant"
    )
    parser.add_argument(
        "--output", type=str, default="casp_results.json", help="Output file for results"
    )
    parser.add_argument(
        "--max-targets", type=int, default=None, help="Maximum targets to evaluate (for testing)"
    )
    ...
    model = AdvancedProteinFoldingModel(
        input_dim=embedder.embed_dim, c_s=384, c_z=128, use_quantum=True
    )
    ...
    # Create benchmark
    benchmark = CASPBenchmark(model=model, embedder=embedder, device=device)

    # Run benchmark
    results = benchmark.benchmark_casp(casp_version=args.casp_version, max_targets=args.max_targets)
