...
class NotebookValidator:
        ...
        if cell_type == "code":
            ...
            if "email" in source.lower() or "@" in source:
                if "example" not in source.lower():
                    self.warnings.append(f"{name}:cell[{idx}]: Possible personal email address")
            ...
            if "](" in source and "github.com" in source:
                ...
                if "] (" in source:  # Space between ] and (
                    self.warnings.append(f"{name}:cell[{idx}]: Possible malformed markdown link")
    ...
    def _check_colab_reliability(self, name: str, cells: List[dict]):
        ...
        if not has_install:
            self.errors.append(f"{name}: Missing dependency installation cell for Colab execution")
