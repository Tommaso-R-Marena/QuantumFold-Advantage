...
## 2025-05-14 - [CI Tagging and File Integrity]
**Learning:** GitHub Actions  using  can produce invalid tags (e.g., starting with a hyphen) when running on detached HEAD or PR merge refs where the branch name is not resolved as expected. Switching to a static prefix like  ensures tag validity. Additionally, mass reformatting or automated edits can occasionally corrupt file structures (e.g., duplicate method signatures, malformed list comprehensions), especially in modules with complex class definitions like `QuantumBackendManager`.
**Action:** Always use static prefixes for image tagging in CI. Mandatory verification of file integrity using `python3 -m compileall` and `black --check` must be performed after any automated or complex code modification to prevent CI regressions.

## 2025-05-14 - [CI Tagging and File Integrity]
**Learning:** GitHub Actions `docker/metadata-action` using `type=sha,prefix={{branch}}-` can produce invalid tags (e.g., starting with a hyphen) when running on detached HEAD or PR merge refs where the branch name is not resolved as expected. Switching to a static prefix like `sha-` ensures tag validity. Additionally, mass reformatting or automated edits can occasionally corrupt file structures (e.g., duplicate method signatures, malformed list comprehensions), especially in modules with complex class definitions like `QuantumBackendManager`.
**Action:** Always use static prefixes for image tagging in CI. Mandatory verification of file integrity using `python3 -m compileall` and `black --check` must be performed after any automated or complex code modification to prevent CI regressions.
